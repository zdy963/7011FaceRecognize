...
from CNN_models import SmallerVGGNet, AlexNet, VGGNet, simpleNet
...
BS = 50                      # Batch size for each epoch
IMAGE_DIMS = (128, 128, 3)    # Spacial dimensions for input images
...
# sorted() sort all the iterable object
# list_images() recursively find all images path in specific directory
imagePaths = sorted(list(paths.list_images(args["dataset"])))
print(len(imagePaths))
random.seed(42)
random.shuffle(imagePaths)      # Shuffle the order for images path
...
for imagePath in imagePaths:
    # print(imagePath)
    try:
        image = cv2.imread(imagePath, COLOR)
        image = cv2.resize(image, (IMAGE_DIMS[1], IMAGE_DIMS[0]))
        image = img_to_array(image)
        data.append(image)

        # imagePath follows: dataset/{CLASS_LABEL}/{FILENAME}.jpg
...
# Simple net
model = simpleNet.build(width=IMAGE_DIMS[1], height=IMAGE_DIMS[0], depth=IMAGE_DIMS[2], classes=len(lb.classes_))
